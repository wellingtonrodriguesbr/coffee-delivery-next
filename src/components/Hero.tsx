...
export function Hero() {
  return (
    <div className="grid grid-cols-2 justify-between items-center mt-24">
      <div className="">
        <h1 className="font-cursive font-extrabold text-5xl leading-snug text-base-title whitespace-nowrap">
        ...
        <div className="grid grid-cols-2 gap-5 mt-16">
          {items.map((item) => (
            <div key={item.text} className="flex items-center gap-2 w-fit">
              <Image src={item.img} alt="" width={32} height={32} priority />
              <span className="text-base whitespace-nowrap text-base-text">
                {item.text}
              </span>
            </div>
          ))}
      ...
      <div className="flex justify-end">
        <Image src={heroImg} alt="" width={476} height={360} priority />
      </div>
    </div>
  );
}
